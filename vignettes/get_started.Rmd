...
As an example, we consider a zone near Buffalo in the Lake Erie. Note that all 
...
To extract depth data from an raster file for our study points, we use `st_extract()` from the package `stars`. Note that the points and the raster must use the same projection.
...
With the `depth` and `fetch`, we create an data frame that contains the predictor value for all points.
...
You can generate predictions using `sav_model(dat)`. By default, the function uses the column names in the input data to identify the relevant variables. If the column names are not recognized, you can manually specify them using the `depth` and `fetch` parameters.
...
Even without additional columns, the model will return columns containing post hoc results. For SAV cover, if presence-absence predictions are available, the cover_post_hoc column will contain cover values set to 0 where absence is predicted.
